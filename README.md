...
2. **Фильтр и агрегация** — по одному nfcapd-файлу запускается **nfdump**:
  - фильтр: `NFDUMP_FILTER` и трафик из ваших сетей `INTERNAL_NETS` (исходящий);
...
**Итог:** детектор смотрит **исходящий** трафик (internal → external), строит топ пар по выбранному критерию (по умолчанию — по числу flow'ов), отфильтровывает пары с числом flow'ов выше порога и шлёт по ним алерты с уровнем угрозы.
